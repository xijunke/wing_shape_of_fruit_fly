...
%%%%%%%%%%%%%%%%%%%%%%%%%
% x =[2.2899,1.0561,0.3788,0.0001];         % 20150204-Hybrid GA-Fmincon计算结果-不错
% x =[3.1133,0.5938,0.3297,0.2450];         % 20150204-计算结果-不错
% x=[2.5088,1.1424,0.7124,0.0458];          % 20150209-计算结果
% x =[2.8632;0.8950;0.1133;0.3662];         % 20150420_wings_morphology_2-结果很好
% x =[2.9992,1.1188,0.8973,0.2395];          % 20150422-hybrid_GA_fminsearch_WingM4_4_1-结果非常不错
% x =[3.1267,1.1578,0.7896,0.1624];  % 20150423_WingM4_4_1-结果非常不错——1
% x =[3.1267,1.1578,0.7896,0.10];      % 20150423_WingM4_4_1-结果非常不错——2
% x =[2.5604,0.9805,0.8083,0.0946];          % 20150424_WingM4_4_1-结果非常不错
R_wing=x(1);
C_aver=x(2);
...
% C_maxy=C_maxyub;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% a_0=-0.413-0.636-C_maxy;  % a_0 =-1.1879;
% b_0=0.1282-0.636-C_maxy;  % b_0 =-0.6467;
%%%%%%%%%%%%%%%%%%%%%%%%%
% 前后缘6阶多项式拟合函数
...
% yr_lead=-0.08249*x.^6+0.9167*x.^5-4.04*x.^4+8.872*x.^3-10.06*x.^2+5.674*x-1.1879;  
% yr_trail=-0.0333*x.^6+0.504*x.^5-2.795*x.^4+7.258*x.^3-8.769*x.^2+3.739*x-0.6467;
...
% yr_leadbem=-0.08249*x_bem.^6+0.9167*x_bem.^5-4.04*x_bem.^4+8.872*x_bem.^3-10.06*x_bem.^2+5.674*x_bem-0.413-x_mod_Root; % yr_leadbem =0.4598;  
% yr_trailbem=-0.0333*x_bem.^6+0.504*x_bem.^5-2.795*x_bem.^4+7.258*x_bem.^3-8.769*x_bem.^2+3.739*x_bem+0.1282-x_mod_Root; % yr_trailbem =-0.6658; 
...
% C_aver=33.7*10^-3;  % m
% AR=2*R/C_aver;
% lambda=AR;   %  lambda =5.9347;
% nu=1.48*10^-5;  % m^2/s   % 在15 °C时,空气的运动粘度是1.48×10?5 m^2/s或14.8 cSt;
% Re=[124,170,200];
% PHI=[46,63,74.5]*pi/180;
% f1=Re(1,1)*nu*lambda/(4*PHI(1,1)*R^2);  % f1 =0.3391; % Hz  
% f2=Re(1,2)*nu*lambda/(4*PHI(1,2)*R^2);  % f2 =0.3395;  % Hz  
% f3=Re(1,3)*nu*lambda/(4*PHI(1,3)*R^2) ; % f3 =0.3378;  % Hz 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% variable=[R_wingeff,C_avereff,xr,C_maxy];      % 变量
% LB = [2, 0.5, 0, 0];      % Lower bound       % 单位: mm & 扭转轴距前缘的无量纲距离
% UB = [4, 2, 2, 0.5];     % Upper bound      % 单位: mm & 扭转轴距前缘的无量纲距离
% R_wing=3.004;  C_aver=0.8854;   % 原始果蝇翅膀
% R_wing=2;  C_aver=0.5;               % 最小翅膀——下限
% R_wing=4;  C_aver=2;                 % 最大翅膀——上限
%%%%%%%%%%%%%%%%%%%%%%%%%
% R_wing=100;  C_aver=33.7;   % mm ——@wing 1
% R_wing=450;  C_aver=237;    % mm  ——@wing 2
% R_wing=0.25*10^3; C_aver=0.0668*10^3;   % mm ——@wing 3
%%%%%%%%%%%%%%%%%%%%%%%%%
% PHI=74.5*pi/180;
% f3 =0.3378;                        % Hz
% xr=0.3289;                           % x-root offset  \mm                    
...
% a six-component force balance
% (ATI NANO-17, Apex, NC, USA). The cylindrical sensor unit (figure 1a; 17 mm diameter, 14.5 mm length) 
% with a mass of 10 g was calibrated to measure a maximum force of 12 N 
% and a maximum torque of 0.5 Nm along the three orthogonal directions.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 绘制拟合好的前后缘
...
% [C_lead_ymax,k_leadmax]=max(f_x_lead);   % 输出: C_lead_ymax=0.4644; k_leadmax=644;% 原始果蝇翅膀@C_maxy=0;
...
% [C_trail_ymin,k_trailmin]=min(f_x_trail);    % 输出: C_trail_ymin =-0.8375;  k_trailmin =409;% 原始果蝇翅膀@C_maxy=0;
...
% x_025=0.25*C_max_LtoT;                               % x_025=0.3255;     % x_025 =0.1154(没意义);
% x_0ub=0.5*C_max_LtoT;                                % x_0ub =0.6509;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (1) R_wing=3.004;  C_aver=0.8854;   % 原始果蝇翅膀
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (2)  for  R_wing=2;  C_aver=0.5;最小翅膀—下限——扭转轴位于最大前缘点和最新后缘点坐标间距0.25倍时@C_maxy=C_maxy25;
% C_lead_ymax=0.2622;  C_trail_ymin=-0.4729; C_max_LtoT =0.7352;  @C_maxy=0;
% C_lead_ymax=0.1838;    C_trail_ymin=-0.5514; C_max_LtoT=0.7352;   @C_maxy=C_maxy25;
% (3)   R_wing=4;  C_aver=2;  最大翅膀—上限——扭转轴位于最大前缘点和最新后缘点坐标间距0.25倍时@C_maxy=C_maxy25;
% C_lead_ymax =1.0490; C_trail_ymin =-1.8917; C_max_LtoT =2.9407;  @C_maxy=0;
% C_lead_ymax =0.7352;    C_trail_ymin =-2.2055; C_max_LtoT =2.9407;  @C_maxy=C_maxy25;  
% (4)   R_wing=100;  C_aver=33.7; 最大翅膀—上限——扭转轴位于最大前缘点和最新后缘点坐标间距0.25倍时@C_maxy=C_maxy25;
% C_lead_ymax =17.6754;   C_trail_ymin = -31.8747; C_max_LtoT=49.5501;  @C_maxy=0;
% C_lead_ymax =12.3877;   C_trail_ymin =-37.1625;    C_max_LtoT=49.5501;  @C_maxy=C_maxy25; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% However, x and y can also be vectors, as explained in the next section. 
% By default, the arrows are scaled to just not overlap, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 输出放大翅膀上的26个点——用于验证转动惯量是否成比例放大
% xr=0.3289;                     % x-root offset  \mm
% % xr_nd=xr/R_wingeff;      % x-root offset  无量纲展向偏置距离
% xr13=xr;
% % xr_nd13=xr_nd;
% r_nd13=linspace(0,1,13);     % x=linspace(R_proximal,R_distal,200);  % x=r_nd*R_wingeff+xr;
% f_x_lead13=f_x_lead1(r_nd13); 
% f_x_trail13=f_x_trail1(r_nd13);
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % 输出翅膀放大之后的前后缘坐标点数据
% r_x13=r_nd13*R_wing+xr13;  % r_x=linspace(0,1,200)*R_wingeff+xr; 
% Rx13=[r_x13';r_x13'];
% f_x=[f_x_lead13';f_x_trail13'];
% A=[Rx13,f_x];  % size(A)=(26*2)
% xlswrite('D:\KXJ\PassiveRot_dynamic_Science_fruitfly\fruitgfly_magnified_wing\wing_magnifiedmod_R.xls',A,'sheet1','A1:B26');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
